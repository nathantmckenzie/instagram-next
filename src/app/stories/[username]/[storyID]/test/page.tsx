...
const App = ({ params }) => {
  ...
  const autoPlay = () => {
    ...
    if (key >= stories[0].stories.length - 1) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await getStory(params.username, params.storyID);
      console.log("response", response.rows);
      let test = [...stories];
      test[0] = { stories: response.rows };
      setStories(test);
      ...
      <div className="flex h-[100vh] flex-row items-center justify-center">
        ...
        {stories
          ? stories.map((story, index) => (
              <div
                key={index}
                className="absolute mb-5 aspect-[9/16] cursor-pointer  overflow-hidden rounded-lg bg-red-400 transition-transform duration-200 ease-in sm:h-[80%] md:h-[93%] md:max-w-[70%]"
                style={{
                  display:
              ...
              >
                ...
                {index === indexSelected ? (
                  <div className="absolute left-11 top-1/2">
                    ...
                    ></i>
                    <button onClick={() => prev(index)}>Previous</button>
                  </div>
                ) : null}
                {index === indexSelected ? (
